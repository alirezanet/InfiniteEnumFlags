...
internal static class HexConverter
{
    ...
    {
        if (inArray is null) throw new ArgumentException(nameof(inArray));
    ...
    public static unsafe string ToString(ReadOnlySpan<byte> bytes, Casing casing = Casing.Upper)
    {
        ...
        Span<char> result = bytes.Length > 16 ? new char[bytes.Length * 2].AsSpan() : stackalloc char[bytes.Length * 2];
